...
const OptionContainer = styled.div`
  ...
  &.active {
    border: solid 1px var(--platform-secondary);
  }
...
function Option({label, icon, optionConnector, addition, pending, setPending}) {
  ...
  return (
    optionConnector === pending.connector && addition === pending.addition ?
      <OptionContainer
        className="flex items-center justify-start text-white rounded-2xl px-5 py-1 my-4"
      >
        <BiRefresh width={18} height={18} color="var(--platform-primary-highlight)" className="mr-3" />
  ...
  useEffect(() =>  {
    if(account || !walletModalOpen) {
      if(account !== undefined) {
        ...
        let arr = [];
        if(accounts){
          arr = JSON.parse(accounts);
          console.log(arr);
          if (!arr.some(item => item === account))
            arr.push(account);
        }
        else {
          arr.push(account);
  ...
  const Footer = () => {
    return (
      <div className="text-white" style={{fontSize: 14}}>
        Don&apos;t have a Wallet?
        <a href="https://metamask.io" target="_blank" rel="noreferrer" className="ml-1" style={{color: 'var(--platform-primary-highlight)'}}>Download Here</a>
      </div>
    )
  }
  ...
  return (
    ...
    :
      ...
      >
        ...
        <Button className="human-green mt-3" onClick={logout}>Logout</Button>
      </Modal>
  )
}
